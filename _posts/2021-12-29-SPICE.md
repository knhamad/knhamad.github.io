...
$N$ is the number of pixels in the image, $M$ is the number of endmembers, $\boldsymbol{X}_{i}$ is the observed pixel, $p_{ik}$ is the proportion of endmember $k$ in pixel $i$, $\boldsymbol{E}_k$ is the $k_{th}$ endmember, and $\varepsilon_i$ is an error term. The LMM assumes that each of the 3 materials, from the previous example, has its own endmember and a proportion value for each endmember in every pixel and combing them produces the seen spectrum. The figure below illustrates this process for a **single** pixel captured by HSC.
